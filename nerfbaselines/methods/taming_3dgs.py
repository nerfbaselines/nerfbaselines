...
from nerfbaselines import (
    Method, MethodInfo, ModelInfo, RenderOutput, Camera, camera_model_to_int, Dataset
)
import torch
import numpy as np
...
class Taming3DGS(Method):
    ...
    def render(self, camera: Camera, *, options=None) -> RenderOutput:
        ...
        viewpoint_cam = _build_caminfo(
            0, camera.pose, camera.intrinsics, f"{0:06d}.png", camera.image_size, scale_coords=self._args.scale_coords)
        render_pkg = render(
