# pylint: disable=unnecessary-lambda-assignment,import-outside-toplevel,import-error
import os
...
from ..distortion import CameraModel


def build_dataset(dataset, config, eval=False):
    ...
    class NBDataset(Dataset):
        ...
        def _load_renderings(self, config):
            ...
            """
            if self.dataset.camera_distortions is not None:
                camtype_map = [
                    camera_utils.ProjectionType.PERSPECTIVE,
                    camera_utils.ProjectionType.PERSPECTIVE,
                    camera_utils.ProjectionType.FISHEYE,
                ]
                self.camtype = [camtype_map[i] for i in self.dataset.camera_distortions.camera_types]
                self.distortion_params = [
                    dict(zip(["k1", "k2", "k3", "k4", "p1", "p2"], self.distortion_params[i])) if self.dataset.camera_distortions.camera_types[i] > 0 else None
                    for i in range(len(self.dataset))
                ]
            else:
                self.distortion_params = [None] * len(self.dataset)
                self.camtype = [camera_utils.ProjectionType.PERSPECTIVE] * len(self.dataset)

            # Scale the inverse intrinsics matrix by the image downsampling factor.
            fx, fy, cx, cy = np.moveaxis(self.dataset.camera_intrinsics, -1, 0)
            pixtocam = np.linalg.inv(np.stack([
                camera_utils.intrinsic_matrix(fx[i], fy[i], cx[i], cy[i])
                for i in range(len(fx))], axis=0))
            ...
            poses = self.dataset.camera_poses
            ...
            self.width, self.height = np.moveaxis(self.dataset.image_sizes, -1, 0)
            if self.dataset.nears_fars is None:
                ...
                self.near, self.far = np.moveaxis(self.dataset.nears_fars, -1, 0)
    if eval:
...
class MultiNeRF(Method):
    ...
    def _load_config(self):
        ...
        from internal import datasets  # pylint: disable=unused-import
        from internal import train_utils  # pylint: disable=unused-import
        import gin
        import jax

        config_paths = []
        gin.parse_config_files_and_bindings(config_paths, [
    ...
    def setup_train(self, train_dataset, *, num_iterations):
        import flax
    ...
    def save(self, path):
        ...
        state_to_save = jax.device_get(
            flax.jax_utils.unreplicate(self.state))
        checkpoints.save_checkpoint(str(path), state_to_save, int(self.step), keep=100)

    def render(self, poses, intrinsics, sizes, nears_fars, distortions, progress_callback=None):
        import flax
        ...
        test_dataset = build_dataset(Dataset(
            camera_poses=poses,
            ...
            camera_distortions=distortions), self.config, eval=True)
        if progress_callback:
            progress_callback(CurrentProgress(0, len(poses), 0, len(poses)))
        for i, test_case in enumerate(test_dataset):
            rendering = models.render_image(
                functools.partial(self.render_eval_pfn, eval_variables, self.train_frac),
                test_case.rays, self.rngs[0], self.config)
            if progress_callback:
