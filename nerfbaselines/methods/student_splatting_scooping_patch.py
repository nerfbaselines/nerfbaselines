...
metrics = {
    "loss": "loss.item()",
    "l1_loss": "Ll1.item()",
    "psnr": "10 * torch.log10(torch.mean(sampling_mask / torch.mean((image - gt_image) ** 2 * sampling_mask))).item()",
    "num_points": "len(primitives.get_xyz)",
}
...
def _(ast_module: ast.Module):
    # Find Scene class
    Scene_ast = single((x for x in ast_module.body if isinstance(x, ast.ClassDef) and x.name == "Scene_nt"))
...
sampling_mask = torch.tensor(1.0)
if viewpoint_cam.sampling_mask is not None:
    sampling_mask = viewpoint_cam.sampling_mask.cuda()
    render_pkg["render"] = render_pkg["render"] * sampling_mask + (1.0 - sampling_mask) * render_pkg["render"].detach()
""").body[0])
    ...
    global_names.remove("checkpoint")
    global_names.remove("first_iter")
    global_names.add("viewpoint_stack")
    ...
    setup_train = [Transformer().visit(x) for x in training_ast.body if x != train_loop]
    # Remove os.system calls
    setup_train = ast_remove_names(setup_train, [
        "network_gui", "tqdm", "prepare_output_and_logger",
        "first_iter", "checkpoint",
    ])
    setup_train_function = cast(ast.FunctionDef, ast.parse("def setup_train(self, args):\n    pass").body[0])
    setup_train_function.body = setup_train[:1] + setup_train[3:]
    ast_module.body.append(setup_train_function)

    # Use this code to debug when integrating new codebase
    ## print("===== Setup train =====") 
    ## print(ast.unparse(setup_train_function.body))
    ## print()
    ## print("===== Train step =====")
