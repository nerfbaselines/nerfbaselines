...
def _(ast_module: ast.Module):
    ...
    load_cam_ast.body.insert(len(load_cam_ast.body)-1, ast.parse("loaded_mask = cam_info.sampling_mask").body[0])
...
def _(ast_module: ast.Module):
    ...
    # Store sampling mask
    init.body.insert(0, ast.parse("self.sampling_mask = gt_alpha_mask.to(self.data_device) if gt_alpha_mask is not None else None").body[0])
...
""").body[0])
    ...
    # 1) Replace all global names with `self`
    # 2) Fix a bug in train.py
    class Transformer(ast.NodeTransformer):
        def __init__(self):
    ...
    training_call = setup_train_start[-8]
    ...
    setup_train0 = [
        ast.parse('self._dataset = self._lp.extract(self._args)').body[0],
        ast.parse('self._opt = self._op.extract(self._args)').body[0],
        ast.parse('self._pipe = self._pp.extract(self._args)').body[0],
        ast.parse('self._debug_from = None').body[0],
    ]
    setup_train = setup_train0 + setup_train
    setup_train = ast_remove_names(setup_train, ["network_gui", "tqdm", "prepare_output_and_logger"])[:-2]
