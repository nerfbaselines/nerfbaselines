...
def dataset_load_features(
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=100) as executor:
        ...
        if dataset["mask_paths"] is not None:
            ...
            logger.debug(f"Loaded {len(masks)} sampling masks")
        ...
        if resize is not None:
            ...
            if dataset["mask_paths"] is not None:
                dataset["mask_paths"] = [
                    os.path.join("/resized-sampling-masks", os.path.relpath(p, dataset["mask_paths_root"])) 
                    for p in dataset["mask_paths"]]
                dataset["mask_paths_root"] = "/resized-sampling-masks"
