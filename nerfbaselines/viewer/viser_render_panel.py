...
class CameraPath:
    ...
    def add_camera(self, keyframe: Keyframe, keyframe_index: Optional[int] = None) -> None:
        ...
        def _(_) -> None:
            ...
            def _(event: viser.GuiEvent) -> None:
                ...
                T_world_target = tf.SE3.from_rotation_and_translation(tf.SO3(keyframe.wxyz), keyframe.position) @ tf.SE3.from_translation(np.array([0.0, 0.0, -0.5]))
    ...
    def interpolate_pose_and_fov_rad(self, normalized_t: float) -> Optional[Tuple[tf.SE3, float]]:
        ...
        self._fov_spline = splines.KochanekBartels(
            [keyframe[0].override_fov_rad if keyframe[0].override_fov_enabled else self.default_fov for keyframe in self._keyframes.values()],
    ...
    def update_spline(self) -> None:
        ...
        self._orientation_spline = splines.quaternion.KochanekBartels(
            [splines.quaternion.UnitQuaternion.from_unit_xyzw(np.roll(keyframe[0].wxyz, shift=-1)) for keyframe in keyframes],
            tcb=(self.tension, 0.0, 0.0),
            endconditions="closed" if self.loop else "natural",
        )
        ...
        points_array = self._position_spline.evaluate(self.spline_t_from_t_sec(np.linspace(0, transition_times_cumsum[-1], num_frames)))
            ...
            def _(_) -> None:
                ...
                with server.add_3d_gui_container(
                    ...
                    override_transition_sec = server.add_gui_number(
                        ...
                        initial_value=keyframe.override_transition_sec if keyframe.override_transition_sec is not None else self.default_transition_sec,
                        min=0.001,
                        max=30.0,
                        step=0.001,
    ...
    def compute_duration(self) -> float:
        ...
        for i, (keyframe, frustum) in enumerate(self._keyframes.values()):
            if i == 0 and not self.loop:
                continue
            del frustum
            total += keyframe.override_transition_sec if keyframe.override_transition_enabled and keyframe.override_transition_sec is not None else self.default_transition_sec
        return total
...
def populate_render_tab(
    ...
    from .viser import VISER_SCALE_RATIO
    ...
    with playback_folder:
        ...
        preview_render_button = server.add_gui_button("Preview Render", hint="Show a preview of the render in the viewport.")
    ...
    def compute_and_update_preview_camera_state() -> Optional[Tuple[tf.SE3, float]]:
        ...
        if preview_frame_slider is None:
            return
        maybe_pose_and_fov_rad = camera_path.interpolate_pose_and_fov_rad(preview_frame_slider.value / get_max_frame_index())
    ...
    load_camera_path_button = server.add_gui_button("Load Path", icon=viser.Icon.FOLDER_OPEN, hint="Load an existing camera path.")

    @load_camera_path_button.on_click
    def _(event: viser.GuiEvent) -> None:
        assert event.client is not None
        camera_path_dir = "camera_paths"
        ...
        with event.client.add_gui_modal("Load Path") as modal:
            if len(preexisting_camera_filenames) == 0:
                ...
                def _(_) -> None:
                    # load the json file
                    json_path = "camera_paths" / camera_path_dropdown.value
                    ...
                    for i in range(len(keyframes)):
                        ...
                        camera_path.add_camera(
                            Keyframe(
                                position=pose.translation() * VISER_SCALE_RATIO,
                                wxyz=pose.rotation().wxyz,
    ...
    def _(event: viser.GuiEvent) -> None:
        ...
        for keyframe, dummy in camera_path._keyframes.values():
            pose = tf.SE3.from_rotation_and_translation(
                ...
                keyframe.position / VISER_SCALE_RATIO,
            )
            keyframes.append(
                {
        ...
        for i in range(num_frames):
            ...
            pose = tf.SE3.from_rotation_and_translation(
                pose.rotation() @ tf.SO3.from_x_radians(np.pi),
                pose.translation() / VISER_SCALE_RATIO,
            )
        ...
        json_outfile = "camera_paths" / f"{render_name_text.value}.json"
