...
from ._websocket import httpserver_websocket_handler
...
def run_simple_http_server(*args, port, **kwargs):
    ...
    def _render_route():
        req = request.json
        frame_bytes, mimetype = backend.render(req)
...
def run_flask_server(*args, port, **kwargs):
    ...
    try:
        with _ViewerBackend(*args, **kwargs) as backend, \
             ThreadingHTTPServer(("", port), partial(ViewerRequestHandler, backend=backend)) as httpd:
...
def run_flask2_server(*args, port, **kwargs):
