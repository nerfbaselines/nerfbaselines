...
METHOD_NAME = "mini-splatting2"


@pytest.fixture
def method_source_code(load_source_code):
    load_source_code("https://github.com/fatPeter/mini-splatting2.git", "5ec9202c2db7d900728bfc09733e85467cff885e")
...
class Rasterizer:
    ...
    def __call__(self, means3D, means2D, shs, colors_precomp, 
                 culling, opacities, scales, rotations, dc,
                 flag_max_count,
                 cov3D_precomp):
        del flag_max_count
        import torch
        ...
        if culling is not None: keep_grad += culling.sum()
        ...
        return rendered_image, radii, observe
...
def method_module(method_source_code, mock_module):
    ...
    diff_rasterization = mock_module("diff_gaussian_rasterization_ms")
    diff_rasterization._C = mock_module("diff_gaussian_rasterization_ms._C")
    fusedssim = unittest.mock.MagicMock()
    def fusedssim(x, y):
        return x.sum() - y.sum()
    fusedssim.fused_ssim = fusedssim
    mock_module("fused_ssim", fusedssim)
    def distCUDA2(x):
        return x.norm(dim=-1)
    ...
    diff_rasterization_2 = mock_module("diff_gaussian_rasterization")
    diff_rasterization_2.GaussianRasterizer = Rasterizer2
    diff_rasterization_2.GaussianRasterizationSettings = argparse.Namespace
...
def _test_method(method_module, colmap_dataset, tmp_path):
    ...
    Method = method_module.MiniSplatting2
...
def test_mini_splatting2_torch(torch_cpu, isolated_modules, method_module, colmap_dataset, tmp_path):
...
@pytest.mark.extras
@pytest.mark.method(METHOD_NAME)
def test_train_mini_splatting2_cpu(torch_cpu, isolated_modules, method_module, run_test_train):
...
def test_train_mini_splatting2_apptainer(run_test_train):
    run_test_train()


@pytest.mark.method(METHOD_NAME)
@pytest.mark.docker
def test_train_mini_splatting2_docker(run_test_train):
    run_test_train()


@pytest.mark.method(METHOD_NAME)
@pytest.mark.conda
def test_train_mini_splatting2_conda(run_test_train):
    run_test_train()
